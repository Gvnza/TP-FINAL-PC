...
def main():
    ...
    pokemon_objects_with_legendaries = define_pokemons_objects_with_legendaries()
    teams = create_teams(50, pokemon_objects)
    rivals = create_teams_with_legendaries(400, pokemon_objects_with_legendaries)
    ...
    for i in range(1, 51):
        ...
        if previous_averages + 7 >= average >= previous_averages - 7:
            if random.random() >= 0.45:
                ...
                rivals = improve_rivals(pokemon_objects_with_legendaries, rivals_results)
                mutated_teams = crossing(resulsts, 50, list(pokemon_objects.values()))
                key_epochs.append(i)
